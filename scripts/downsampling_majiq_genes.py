...
output = sys.argv[5] #	"_x_genes_in_y_samples"
    ...
    for line in majiq_file:
        if line.split()[0] != 'Gene':
            ...
            if exp_level[gene_id] == 'low':
                gene_set_low.add(gene_id)
            elif exp_level[gene_id] == 'mid':
                gene_set_mid.add(gene_id)
            if  exp_level[gene_id] == 'high':
                gene_set_high.add(gene_id)
    majiq_file.close()
...
def Counting(gene_list, explevel):
    ...
    for k,v in number_genes_counter.items():
        x_genes_in_y_samples = open(sys.argv[5], 'a')
        result = '\t'.join((str(k), str(v), explevel))
        x_genes_in_y_samples.write(result + '\n')
        x_genes_in_y_samples.close()
